...
class DBSpecViewSet(viewsets.AuditedModelViewSet):
    ...
    def update(self, request, *args, **kwargs):
        ...
        for key in update_data:
            ...
            # 判断磁盘的变化
                ...
                for new_storage in update_data["storage_spec"]:
                    flag = False
                    for old_storage in spec.storage_spec:
                        if new_storage["mount_point"] == old_storage["mount_point"]:
                            flag = True
    ...
    def query_qps_range(self, request, *args, **kwargs):
        ...
        qps_min_max_map = {
            spec.qps.get("min", 0)
            * math.ceil(data["capacity"] / spec.capacity): spec.qps.get("max", 0)
