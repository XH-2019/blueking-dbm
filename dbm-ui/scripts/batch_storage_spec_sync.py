    ...
    with transaction.atomic():
        ...
        for spec in all_spec:
            ...
            for storage_spec in spec.storage_spec:
                if storage_spec.get("min") and storage_spec.get("max") and not storage_spec.get("size"):
                    new_storage_spec.append(storage_spec)
                    continue
                storage_spec["min"] = storage_spec.pop("size") if storage_spec.get("size") else storage_spec.get("min", 1)
